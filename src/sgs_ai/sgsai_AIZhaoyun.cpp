...
sgs::DataType::Message * AIZhaoyun::useCardResponse() {
	sgs::Derive::SkillMessage * longdan = useLongdanInUsecard();
	if (longdan) {
...
sgs::DataType::Message * AIZhaoyun::shaOrNot(sgs::DataType::Player * from,
	...
	sgs::Derive::SkillMessage * longdan = useLongdanInShaOrNot();
	if (longdan) {
		printDebug(sgsui::messageInterpret(longdan));
		printDebug(
...
sgs::DataType::Message * AIZhaoyun::shanOrNot(sgs::DataType::Player * from,
		const sgs::DataType::Card * card) {
	sgs::Derive::SkillMessage * longdan = useLongdanInShanOrNot();
	if (longdan) {
		printDebug(sgsui::messageInterpret(longdan));
		printDebug(
...
sgs::Derive::SkillMessage * AIZhaoyun::useLongdanInUsecard() {
	sgs::Derive::LongDanMessage * returnLongdan = 0;
	if (!canUseSha())
	...
	if (attackTargetVec.empty()) {
		for (CardVec::const_iterator cardIter = shanCardFound.begin();
			...
			if (canUseFangtian() && attackTargetVec.size() > 1) {
				...
				returnLongdan = new sgs::Derive::LongDanMessage(myPlayer(),
						(*cardIter).second);
...
sgs::Derive::SkillMessage * AIZhaoyun::useLongdanInShaOrNot() {
	sgs::Derive::LongDanMessage * returnLongdan = 0;
...
sgs::Derive::SkillMessage * AIZhaoyun::useLongdanInShanOrNot() {
	sgs::Derive::LongDanMessage * returnLongdan = 0;

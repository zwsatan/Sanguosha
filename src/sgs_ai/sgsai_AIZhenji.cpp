...
sgs::DataType::Message * AIZhenji::shanOrNot(sgs::DataType::Player * from,
	...
	sgs::Derive::SkillMessage * qingguo = useQingguoInShanOrNot();
	if (qingguo) {
...
sgs::Derive::SkillMessage * AIZhenji::useQingguoInShanOrNot() {
	sgs::Derive::QingGuoMeesage * returnQingguo = 0;
	...
	for (CardVec::const_iterator cardIter = blackShoupaiFound.begin();
		...
		returnQingguo = new sgs::Derive::QingGuoMeesage(myPlayer(),
				(*cardIter).second);

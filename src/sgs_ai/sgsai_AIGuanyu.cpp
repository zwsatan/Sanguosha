...
sgs::DataType::Message * AIGuanyu::useCardResponse() {
	sgs::Derive::SkillMessage * wusheng = useWushengInUsecard();
	if (wusheng) {
...
sgs::DataType::Message * AIGuanyu::shaOrNot(sgs::DataType::Player * from,
	...
	sgs::Derive::SkillMessage * wusheng = useWushengInShaOrNot();
	if (wusheng) {
		printDebug(sgsui::messageInterpret(wusheng));
		printDebug(
...
sgs::Derive::SkillMessage * AIGuanyu::useWushengInUsecard() {
	sgs::Derive::WuShengMessage * returnWusheng = 0;
	if (!canUseSha())
	...
	if (attackTargetVec.empty()) {
		for (CardVec::const_iterator cardIter = redShoupaiFound.begin();
			...
			if (canUseFangtian() && attackTargetVec.size() > 1) {
				...
				returnWusheng = new sgs::Derive::WuShengMessage(myPlayer(),
...
sgs::Derive::SkillMessage * AIGuanyu::useWushengInShaOrNot() {
	sgs::Derive::WuShengMessage * returnWusheng = 0;

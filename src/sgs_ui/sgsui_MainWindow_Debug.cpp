...
void MainWindow::initiateDebug()
{
	...
	connect(refreshHistoryButton, SIGNAL(clicked()), this, SLOT(refreshHistory()));
...
void MainWindow::debug3()
{
	...
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[0], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[1], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[2], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[3], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[4], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[5], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[6], true));
	temp.push_back(std::pair<const sgs::DataType::Card *, bool>(&card[7], true));
	showWugu(temp);
}
...
void MainWindow::debug4()
{
	if (isBlocked())
	{
		printDebug("<font color=red><b>Warning: </b></font>"
				   "MainWindow::debug4: MainWindow is still blocking!");
		return;
	}
...
void MainWindow::refreshPlayer()
{
	...
	if (GUIStaticData::isPlayersCreated())
	{
		...
		if (playerSeat + 1 <= GUIStaticData::getPlayerCount())
		{
			...
			playerText->append("player " + QString::number(playerSeat) + wujiangDisplayName(player->type()));
...
void MainWindow::refreshData()
{
	...
	if (GUIStaticData::isPlayersCreated())
	{
		...
		for (int i = 0; i < cardDeckSize; ++i)
		{
			...
			QString cardText(QString::number(i + 1) + QString(i < 9 ? "  " : " "));
			if (i < 9)
				cardText.append("  ");
			else
				cardText.append(" ");
		...
		sgs::DataType::Player * player = 0;
		for (int index = 1, playerCount = GUIStaticData::getPlayerCount(); index <= playerCount; ++index)
		{
		...
		allCardStr.append(QString::number(playerCardNum + cardDeckSize + dustbinSize));
		...
		for (int i = 0; i < 108; ++i)
		{
			...
			for (auto iter = totalCardList.begin(); iter != totalCardList.end(); ++iter)
			{
				if (*iter == thiscard)
				{
					thiscardExist = true;
		...
		for (auto iter = missingCardList.begin(); iter != missingCardList.end(); ++iter)
		{
...
void MainWindow::refreshHistory()
{
	...
	if (GUIStaticData::isPlayersCreated())
	{
		...
		auto citer = GUIStaticData::getHistoryVec().rbegin();
		for (int i = 0; i < count && citer != GUIStaticData::getHistoryVec().rend(); ++citer)
		{
			if (messageType == messageTypeNone || (*citer)->type() == messageType)
			{
...
void MainWindow::setHPDebug()
{
	...
	if (GUIStaticData::isPlayersCreated())
	{
		...
		if (playerSeat < GUIStaticData::getPlayerCount())
		{
			...
			sgs::DataType::Player * player = sgs::ExternData::gamedata.players(playerSeat);
